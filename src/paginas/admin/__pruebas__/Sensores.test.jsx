...
vi.mock("../../../utilidades/api", () => ({ crearSensor: vi.fn(), actualizarActivoSensor: vi.fn() }));
...
describe("Sensores (catálogo de nodos)", () => {
  ...
  it("hacer clic en el estado alterna activo/inactivo llamando a actualizarActivoSensor", async () => {
    const setData = vi.fn();
    useSensores.mockReturnValue({
      data: [{ id: "s1", codigo: "RIO-PIURA-01", nombre: "Puente Bolognesi", activo: true }],
      loading: false,
    ...
    actualizarActivoSensor.mockResolvedValue({ id: "s1", activo: false });
    ...
    expect(actualizarActivoSensor).toHaveBeenCalledWith("s1", false);
    expect(setData).toHaveBeenCalled();
  });
});
