import { obtenerTokenGuardado, dispararSesionExpirada } from "../contexto/AuthContext";
...
export function crearAlbergue({ nombre, direccion, capacidad, lon, lat }) {
  return apiFetch("/api/albergues", {
    method: "POST",
    body: JSON.stringify({ nombre, direccion, capacidad, lon, lat }),
  });
}

export function eliminarAlbergue(albergueId) {
  return apiFetch(`/api/albergues/${albergueId}`, { method: "DELETE" });
}
...
export function actualizarActivoSensor(sensorId, activo) {
  return apiFetch(`/api/sensores/${sensorId}/activo`, {
    method: "PATCH",
...
export function getPlanesSeguro() {
  return apiFetch("/api/polizas/planes");
}

export function crearCheckoutSeguro(meses) {
  return apiFetch("/api/polizas/checkout", {
    method: "POST",
    body: JSON.stringify({ meses }),
  });
}

export function getMiPoliza() {
  return apiFetch("/api/polizas/mia");
}

export function preguntarAsistente({ pregunta, historial }) {
  return apiFetch("/api/asistente/preguntar", {
    method: "POST",
    body: JSON.stringify({ pregunta, historial }),
  });
}

export function enviarFeedbackAsistente({ pregunta, respuesta, util }) {
  return apiFetch("/api/asistente/feedback", {
    method: "POST",
    body: JSON.stringify({ pregunta, respuesta, util }),
  });
}

export function getFeedbackAsistente() {
  return apiFetch("/api/asistente/feedback");
}
